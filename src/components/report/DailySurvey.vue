           ...
           class="table">
            ...
            <tbody>
              ...
              <tr>
                ...
                <td>风险损伤位置：{{typeof(equipStateInfo.damageLocation) == 'undefined' ? '' : equipStateInfo.damageLocation}}</td>
              </tr>
              <tr>
              ...
              <tr>
                <td colspan="2" style="color:#0099FF;">责任人：{{equipStateInfo.contactPerson}}</td>
              </tr>
            </tbody>
          </table>
        </el-col>
...
export default {
  ...
  created() {
    eventBus.on('getCode', (data) => {
      if(!data.isWeekDisable) {
        this.getEquipInfo(data.currentEquipCode,data.reportDate);
      }
    })
  },
  mounted() {
  ...
  methods: {
    async getEquipInfo(equipmentCode,reportDate) {
      this.equipStateInfo = {};
      const res = await this.$http.post(this.$urlObj.queryImage,{equipmentCode: equipmentCode,reportDate:reportDate});
